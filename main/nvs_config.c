...
void nvs_config_save_pwm_params(void) {
    ESP_LOGI(TAG, "nvs_config_save_pwm_params called: period=%u, duty=%u, interval=%u",
             s_config.peltier_pwm_period, s_config.peltier_pwm_duty, s_config.peltier_pwm_interval);

    nvs_handle_t handle;
    if (nvs_open(NVS_NAMESPACE, NVS_READWRITE, &handle) != ESP_OK) {
        ESP_LOGE(TAG, "Failed to open NVS for PWM params save");
        return;
    }
    ...
    if (err != ESP_OK) {
        ...
        ESP_LOGI(TAG, "Saved peltier_pwm_period to NVS: %u", s_config.peltier_pwm_period);
    }

    err = nvs_set_u8(handle, NVS_KEY_PELTIER_PWM_DUTY, s_config.peltier_pwm_duty);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to save peltier_pwm_duty: %s", esp_err_to_name(err));
    } else {
        ESP_LOGI(TAG, "Saved peltier_pwm_duty to NVS: %u", s_config.peltier_pwm_duty);
    }

    err = nvs_set_u16(handle, NVS_KEY_PELTIER_PWM_INTERVAL, s_config.peltier_pwm_interval);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to save peltier_pwm_interval: %s", esp_err_to_name(err));
    } else {
        ESP_LOGI(TAG, "Saved peltier_pwm_interval to NVS: %u", s_config.peltier_pwm_interval);
    }
    ...
    if (err != ESP_OK) {
        ...
        ESP_LOGI(TAG, "PWM params saved to NVS successfully");
    }
